module NpSearch
	...
	## => Not sure whether to include since firstly, there is a paper that says that letters that do not strictly code for anything are sometimes used (such as "j" and "o" also stand for a specific case of an amino acid). Also cannot check for other characters (such as empty space) since the motif could be a regex...
	    ## => #Give user option to ignore - since the file may contain weird lettes that the user may wanna search for.
	    def motif_validator(motif, input_type)
	#       if motif.match(/[bjouxz]/) # checks whether the motif contains only amino acid residues and not other junk.  
	#          if input_type == "protein" 
	#               puts "The motif query does not seem to contain only protein residues"
	#           elsif input_type == "dna"
	#               abort "\nThe motif query must only contain protein residues (i.e. only the following ACDEFGHIKLMNPQRSTVWY).\n This is because the genetic data will be translated in all 6 frames before being searched for the query motif. \n"
	#           end
	#       end
	    end
